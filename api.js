...
function displayPlanets(planetDataArray, planetName) {
    ...
    planetDataArray.forEach(item => {
        if (item.key === "Distance from Sun (minimum / maximum)" || item.key === "Mean distance from earth") {
        ...
        } else if (item.key === "Surface Temperature (mean)" || item.key === "Temperature (core)") {
...
async function fetchPlanets(planetName) {
    ...
    const apiUrl = `http://10.48.1.142:3000/api/space/solar-system/${planetName}`;
